...
function configureBrevo() {
  if (!brevoConfigured) {
    if (!process.env.BREVO_API_KEY) {
      console.error('⚠️ Brevo API key not configured! Email features will not work.');
...
export async function sendLiveEventNotification(email, event) {
  try {
    ...
    sendSmtpEmail.sender = getSender();
    sendSmtpEmail.to = [{ email }];
    sendSmtpEmail.subject = `🔴 LIVE NOW: ${event.title}`;
...
export async function sendVerificationEmail(email, name, token) {
  try {
    ...
    sendSmtpEmail.sender = getSender();
    sendSmtpEmail.to = [{ email }];
    sendSmtpEmail.subject = '✨ Verify Your Email - Welcome to the Fan Club!';
